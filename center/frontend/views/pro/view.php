...
<main id="main" class="gray">
    <div class="container">
        ...
        出厂价   <?= Yii::$app->user->can('pricey');?>  <br>
       代理价   <?= Yii::$app->user->can('priced');?><br>
              <?=  $model->content  ?>
         <small class="fr">创建时间：    <?= Yii::$app->formatter->asDate( $model->created_at,'long')  ?></small>
           ...
           <div class="next-article">
                <div class="row">
		<div class="col-sm-12 col-md-6">   <?php if($prev){ ?>   <a href="<?= Yii::$app->urlManager->createUrl(['pro/view','id'=>$prev->id]); ?>">上一篇： <?= $prev->title; ?> </a>
                <?php }else{ echo "暂无";} ?>
            </div>
        <div class="col-sm-12 col-md-6">  
            <?php if($next){ ?>  <a href="<?= Yii::$app->urlManager->createUrl(['pro/view','id'=>$next->id]); ?>"> 下一篇：<?=  $next->title; ?>  </a>
            <?php }else{ echo "暂无";} ?>
        </div> 
